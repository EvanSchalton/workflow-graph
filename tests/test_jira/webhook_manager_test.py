...
async def test_broadcast(webhook_manager, mock_session):
    ...
    mock_event = BoardEvent(event_code=EventCode.BOARD_CREATE, payload=mock_board)
    
    # Call the broadcast method
    await webhook_manager.broadcast(mock_event)
    
    # Verify the publish method was called on each webhook
    webhook_manager.get_subscribers.assert_called_once_with(mock_event.event_code)
    mock_webhook1.publish.assert_called_once_with(event=mock_event)
    mock_webhook2.publish.assert_called_once_with(event=mock_event)
