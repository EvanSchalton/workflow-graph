...
async def create_webhook(webhook: Webhook, session: Session = Depends(get_session)):
    session.add(webhook)
...
async def read_webhooks(session: Session = Depends(get_session)):
    result = await session.execute(select(Webhook))
    return result.scalars().all()

@router.get("/{webhook_id}", response_model=Webhook)
async def read_webhook(webhook_id: int, session: Session = Depends(get_session)):
...
async def update_webhook(webhook_id: int, webhook: WebhookUpdate, session: Session = Depends(get_session)):
...
async def delete_webhook(webhook_id: int, session: Session = Depends(get_session)):
    webhook = await session.get(Webhook, webhook_id)
    if not webhook:
        raise HTTPException(status_code=404, detail="Webhook not found")
